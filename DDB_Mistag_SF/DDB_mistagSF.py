...
def withoutSingleTopAndInclusive(x, year_, isTope):
    if isTope == "True":
        print "Top (e) Control Region"
    else:
        print "Top (mu) Control Region"
    print "Running Without Single Top for Inclusive Analysis of " + str(year_) + " Data"
    print ""
    
    vers = yearandversion(year_)
    
    outdirtype = "withoutSingleTop/"
    outputpath = vers[2]+outdirtype+x+"/"
    if not os.path.exists(vers[2]):
        os.mkdir(vers[2])
    if not os.path.exists(vers[2]+outdirtype):
    ...
    SubtractedData = h_Data.Clone("SubtractedData")
    SubtractedData = SubtractedData - (h_sumWJets + h_sumDiboson)
    SubtractedDataPassed = h_Data_Passed.Clone("SubtractedDataPassed")
    SubtractedDataPassed = SubtractedDataPassed - (h_sumWJetsPassed + h_sumDibosonPassed)
    SubtractedDataFailed = h_Data_Failed.Clone("SubtractedDataFailed")
    SubtractedDataFailed = SubtractedDataFailed - (h_sumWJetsFailed + h_sumDibosonFailed)
    ...
    row14 = ["Background", "Pass (normalized)", str(round((h_sumWJetsPassed + h_sumDibosonPassed).Integral(),2))]
    row15 = [" ", "Fail (normalized)", str(round((h_sumWJetsFailed + h_sumDibosonFailed).Integral(),2))]
    row16 = [" ", "Total (normalized)", str(round((h_sumWJets + h_sumDiboson).Integral(),2))]
...
def withoutSingleTopAndMETbins(lowerMET, upperMET, year_, isTope):
    if isTope == "True":
        print "Top (e) Control Region"
    else:
        print "Top (mu) Control Region"
    print "Running Without Single Top for MET " + str(lowerMET) + "-" + str(upperMET) + " GeV Analysis of " + str(year_) + " Data"
    print ""
    
    vers = yearandversion(year_)
    
    outdirtype = "withoutSingleTop/MET"
    outputpath = vers[2]+outdirtype+str(lowerMET)+"-"+str(upperMET)+"/"
    if not os.path.exists(vers[2]):
        os.mkdir(vers[2])
    if not os.path.exists(vers[2]+outdirtype):
    ...
    #print " "
    if isTope == "True":
        print "relative statistical Uncertainty in Top (e) CR", statUnc/SF*100, " %"
    else:
        print "relative statistical Uncertainty in Top (mu) CR", statUnc/SF*100, " %"
    print " "
...
def withoutSingleTopAndpTbins(lowerpT, upperpT, year_, isTope):
    if isTope == "True":
        print "Top (e) Control Region"
    else:
        print "Top (mu) Control Region"
    print "Running Without Single Top for pT " + str(lowerpT) + "-" + str(upperpT) + " GeV Analysis of " + str(year_) + " Data"
    print ""
    
    vers = yearandversion(year_)
    
    outdirtype = "withoutSingleTop/pT"
    outputpath = vers[2]+outdirtype+str(lowerpT)+"-"+str(upperpT)+"/"
    if not os.path.exists(vers[2]):
        os.mkdir(vers[2])
    if not os.path.exists(vers[2]+outdirtype):
...
if __name__ == '__main__':
    ...
    if args.discrepancy:
        if args.isTope:
            if args.WOI:
                withoutSingleTopAndInclusive(str(args.WOI), int(args.discrepancy), args.isTope)
        ...
        if args.WOMET:
            if args.isTope:
                inbins = (args.WOMET).split("-")
                withoutSingleTopAndMETbins(int(inbins[0]), int(inbins[1]), int(args.discrepancy), args.isTope)
    
        if args.WOPT:
            if args.isTope:
                inbins = (args.WOPT).split("-")
                withoutSingleTopAndpTbins(int(inbins[0]), int(inbins[1]), int(args.discrepancy), args.isTope)
    
        if args.WMET:
            if args.isTope:
                inbins = (args.WMET).split("-")
                withSingleTopAndMETbins(int(inbins[0]), int(inbins[1]), int(args.discrepancy), args.isTope)

        if args.WPT:
            if args.isTope:
                inbins = (args.WPT).split("-")
                withSingleTopAndpTbins(int(inbins[0]), int(inbins[1]), int(args.discrepancy), args.isTope)
